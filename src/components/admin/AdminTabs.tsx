...
interface AdminTabsProps {
    activeTab: 'menu' | 'template' | 'customize' | 'settings' | 'membership';
    slug: string;
    onTabChange?: (tab: 'menu' | 'template' | 'customize' | 'settings' | 'membership') => void;
}

export function AdminTabs({ activeTab, slug, onTabChange }: AdminTabsProps) {
    const router = useRouter();

    const handleNavigation = (tab: 'menu' | 'template' | 'customize' | 'settings' | 'membership') => {
        if (onTabChange) {
            onTabChange(tab);
        }
